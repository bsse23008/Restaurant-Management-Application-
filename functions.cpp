...
string deliveryRiders[5] = {"Ali", "Ahmad", "AKBAR", "Taqi", "Wasif"};
int rating;
...
bool loginAuth(T userName, T pass) {
  ...
  for (int i = 0; i < noOfUsers; ++i) {
    if ((userName == usersInfo[i][1] || userName == usersInfo[i][2]) &&
        pass == usersInfo[i][3]) {
      activeUser = i;
      return true;
    }
...
int SignupPage() {
  ...
  if (output.is_open()) {
    output << fullName << " " << userName << " " << email << " " << password
           << " " << roles[type - 1] << endl;
  }
  return 0;
}
...
template <class T, class K> struct Menu {
  T name;
  K price, discount;
};
...
void viewMenu() {
  ...
  if (data.is_open()) {
    ...
    cout << setw(20) << "Food Items" << setw(20) << "\tPrice" << setw(15)
         << "\tDiscount" << endl;
    for (int i = 0; i < j - 1; i++) {
      cout << setw(2) << i + 1 << ". " << setw(15) << menu[i].name << setw(20)
           << menu[i].price << " Rs/- only" << setw(15) << menu[i].discount
           << endl;
    }
...
void deleteProduct(int noItems) {
  ...
  if (input.is_open()) {
    while (!input.eof()) {
      input >> menu[j].name >> menu[j].price >> menu[j].discount;
      j++;
    }
  } else {
    cout << "\nNot Open";
  ...
  for (int i = 0; i < noItems - 1; i++) {
    cout << i + 1 << ".  " << menu[i].name << setw(5) << menu[i].price
         << setw(5) << menu[i].discount << endl;
  }
  ofstream data("menu.txt", ios::out);
  if (data.is_open()) {
    for (int i = 0; i < noItems - 1; i++) {
      data << menu[i].name << " " << menu[i].price << " " << menu[i].discount
           << endl;
    }
  }
  delete[] menu;
...
template <typename T, typename K> struct Stock {
  T name;
  K quantity;
};
...
void adminPage() {
  ...
  if (check == 1) {
    ...
    if (output.is_open()) {
      for (int i = 0; i < num; i++) {
        output << arr1[i].name << "  " << arr1[i].quantity << endl;
      }
    }
    output.close();
    viewStock();
...
void chefPage() {
  ...
  while (true) {
    cout << "\n1. To add recipe \n2. To review order \n3. To View Stock \n4. "
            "To Edit Stock"
         << endl;
    cout << "From The Above Options Enter The Task Number that you want to "
...
void profit(const int name[], const int sale[], int item, const int amount[]) {
  ...
  if (file.is_open()) {
    while (true) {
      file >> quantity[k].name >> quantity[k].price;
      k++;
      if (file.eof()) {
        break;
      }
...
void collectFeedback(Feedback &feedback, double chefSalary) {
  cout << "Please rate the food on a scale of 1 to 5 (1 = Poor, 5 = "
          "Excellent): ";
  ...
  cout << "Please rate your overall dining experience (1 = Poor, 5 = "
          "Excellent): ";
  cin >> feedback.overallExperienceRating;
  if (feedback.overallExperienceRating < 1 ||
      feedback.overallExperienceRating > 5) {
    cout << "Invalid rating. Please enter a number between 1 and 5." << endl;
    return;
  }
  cin.ignore();
  ...
  cout << "Please rate the food prepared by the chef on a scale of 1 to 5 (1 = "
          "Poor, 5 = Excellent): ";
  cin >> feedback.foodRating;
  ...
  feedback.customerPoints =
      customer_discount_rate * feedback.overallExperienceRating * 100;
}
...
void saveFeedback(Feedback &feedback) {
  ...
  feedbackFile << "Overall Experience Rating: "
               << feedback.overallExperienceRating << endl;
...
void feedback() {
  ...
  while (numCustomers < maxCustomers) {
    ...
    cout << "Please provide your contact information for special offers:"
         << endl;
    cout << "Name: ";
    ...
    cout << "Thank you for your feedback and contact information! It has been "
            "recorded."
         << endl;
    cout << "Enter -1 to finish entering feedback, or continue for the next "
            "customer."
         << endl;
  }
  ...
  for (int i = 0; i < numCustomers; ++i) {
    cout << "Customer Points for " << feedbackData[i].customerName << ": "
         << feedbackData[i].customerPoints << endl;
  }
}
...
int userPage() {
  ...
  for (int a = 0; a < noOfOrder; a++) {
    num[a] = num[a]; // for original index of Items now num[a] has original
                     // index
  }
  for (int a = 0; a < noOfOrder; a++) {
    num[a] =
        num[a] - 1; // for original index of Items now num[a] has original index
  }
  ...
  for (int a = 0; a < noOfOrder; a++) {
    ...
    totalPrice[a] = menu[num[a]].price - totalDiscount[a]; // final price of
                                                           // item
    cout << "\n amount " << amount[a];
  ...
  for (int a = 0; a < noOfOrder; a++) {
    ...
    cout << a + 1 << "." << menu[num[a]].name << setw(17)
         << menu[num[a]].discount << "%" << setw(15) << amount[a] << setw(17)
         << totalPrice[a] << setw(20) << priceAfterTax << "\n";
  }
  cout << "\t\tTotal Bill:" << priceAfterTax;

  while (true) {
    cout << "\nPress 1 to select delivery and 0 to dine in (negative value to "
            "escape) :  "
         << endl;
    cin.ignore();
    cin >> y;
    if (y < 0) {
      ...
      cout << "The phone number of the customer is  : " << F.phoneNumber
           << endl;
      cout << "The selected items and payable amount is  : " << endl;
      for (int a = 0; a < noOfOrder; a++) {
        cout << a + 1 << "." << menu[num[a]].name << "\t\t"
             << menu[num[a]].discount << "%"
             << "\t\t" << totalPrice[a] << "\n";
      }
      while (true) {
        cout << "\nEnter the rider rating from (1-5) according to the customer "
                "reviews "
             << endl;
        cin >> rating;
      ...
      if (order.is_open()) {
        ...
        for (int a = 0; a < noOfOrder; a++) {
          order << a + 1 << "." << menu[num[a]].name << "\t\t" << endl;
        }
      }
      cout << "Complete info of order for table num " << x
           << "has been sent to kitchen department " << endl;
...
template <typename T> T calculateSalesTax(T itemPrice, T taxRate) {
...
bool isValidDate(string &date) {
  if (date.length() != 10 || date[2] != '-' || date[5] != '-' ||
      ...
      (date[0] == '1' && (date[1] < '0' || date[1] > '2')) || date[3] < '0' ||
      date[3] > '3' || (date[3] == '0' && (date[4] < '1' || date[4] > '9')) ||
      (date[3] == '3' && (date[4] != '0')) || date[6] != '2' ||
      date[7] != '0' || date[8] != '2' || date[9] != '3') {
    return false;
  } else {
    return true;
  }
...
bool isValidTime(string &time) {
  if (time[0] > '2' || (time[0] == '2' && time[1] > '3') || time[2] != ':' ||
      time[3] > '5' || (time[3] == '5' && time[4] > '9')) {
    return false;
  }
  return true;
}
...
bool dataStore(string date1, string time1, int table1) {
...

  {
    ...
    while (!read.eof() && i < noOfExistingRes + 1) {
      ...
      read >> name[i] >> name[i] >> date[i] >> time[i] >> name[i] >> tableNo[i];
      i++;
    }
  }
...
void tableReservation() {
  ...
  while (true) {
    ...
    if (cin >> n.noOfGuests) {
      if (n.noOfGuests > 10) {
        cout << "Sorry! We can't accommodate more than 10 people at a time."
             << endl;
      } else {
        break;
      }
  ...
  while (true) {
    ...
    if (isValidContactNumber(n.phoneNo)) {
      ...
      cout << "Invalid Number. Please enter a valid number in the format "
              "0300-0000000.\n";
    }
  }
  ...
  if (!isAvailable) {
    ...
    if (!res1.is_open()) {
      ...
      res1 << n.name << " " << n.noOfGuests << " " << n.date << " " << n.time
           << " " << n.phoneNo << " " << n.tableNo << endl;
      res1.flush();
      res1.close();
      ...
      cout << "\n\n\t\t===================================\n";
      cout << "\t\t  Your Total Amount Due is : " << calculateBill
           << "\t(1500 each person)" << endl;
      cout << "\t\t====================================" << endl;
    }
  } else {
    cout << "Sorry, the table is already reserved for the desired date and "
            "time.\n";
